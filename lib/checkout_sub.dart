...
//Billing window in checkout page.
                      ...
                      InkWell(
                        onTap: () async {
                          ...
                          var cart = (await Firestore.instance
                              ...
                              .documents;
                          int quantity;
                          List<DocumentReference> varId =
                              List<DocumentReference>();
                          for (int x = 0; x < cart.length; x++) {
                            DocumentSnapshot products =
                                (await cart[x].data['product'].get());
                            var variations = (await Firestore.instance
                                    .collection('products_variations')
                                    .where('product',
                                        isEqualTo: cart[x].data['product'])
                                    .getDocuments())
                                .documents;
                            var options = (await Firestore.instance
                                    .collection('cart_variations')
                                    .where('cart_id',
                                        isEqualTo: cart[x].reference)
                                    .getDocuments())
                                .documents;
                            quantity = cart[x].data['quantity'];
                            var varOp = (await Firestore.instance
                            ...
                            for (int y = 0; y < variations.length; y++) {
                              newVal[
                                  variations[y].data['variation_desc'] +
                                      x.toString()] = options[0]
                                  .data[variations[y].data['variation_desc']];
                            }
                            for (int y = 0; y < varOp.length; y++) {
                              bool found = true;
                              for (int z = 0; z < variations.length; z++) {
                                if (varOp[y].data[
                                        variations[z].data['variation_desc']] !=
                                    options[0].data[
                                        variations[z].data['variation_desc']]) {
                                  found = false;
                                  break;
                            ...
                            newVal['productName' + x.toString()] =
                                products.data['prod_name'];
                            newVal['product' + x.toString()] =
                                products.reference;
                          ...
                          for (int x = 0; x < varId.length; x++) {
                            Firestore.instance
                                .collection('cart_variations')
                                .document(varId[x].documentID)
                                .delete();
                          }
                          for (int x = 0; x < cart.length; x++) {
                            Firestore.instance
                                .collection('shopping_cart')
                                .document(cart[x].documentID)
                                .delete();
                          }
